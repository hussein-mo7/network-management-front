...
export function ImportSubscribersModal({
  ...
  return (
    <Modal
      open={open}
      onClose={handleClose}
      title={t("settings.excel.subscribers.importModalTitle")}
      ...
      footer={
        <>
          ...
          <ModalFooterButton onClick={handleImport} disabled={!file} isLoading={isSubmitting}>
            {t("settings.excel.subscribers.importSubmit")}
          </ModalFooterButton>
        </>
      }
    >
      <div className="space-y-4">
        <Text muted className="text-sm">
          {t("settings.excel.subscribers.importHint")}
        </Text>
        ...
        >
          ...
          <span className="text-sm font-medium text-foreground">
            {file ? file.name : t("settings.excel.subscribers.selectFile")}
          </span>
